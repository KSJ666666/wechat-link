...
public class QueryAttractionsTool implements FunctionTool {
    ...
    public QueryAttractionsTool(AmapService amapService) {
        this.amapService = amapService;
    }

    @Override
    public String name() {
        return "query_attractions";
    }

    @Override
    public String description() {
        return "查询某城市的景点/景区列表。"
                + "当用户询问'有什么景点、景区、哪里好玩、适合去哪玩'等需要推荐景点的问题时调用。"
                + "可用于攻略选点和行程规划。";
    }
    ...
    public JsonNode parameters() {
        ...
        props.putObject("city")
                .put("type", "string")
                .put("description", "城市名或拼音（必填，如：北京、hangzhou）");
        props.putObject("keyword")
                .put("type", "string")
                .put("description", "具体景点名（可选，如：西湖、故宫），不填则返回该城市的景点列表");
