...
public class SearchPoiTool implements FunctionTool {
    ...
    public SearchPoiTool(AmapService amapService) { this.amapService = amapService; }
    @Override public String name() { return "search_poi"; }
    @Override public String description() { return "按关键词搜索某城市的地点（如餐厅、酒店、景点、商场）。"; }
    @Override public JsonNode parameters() {
        ObjectNode p = MAPPER.createObjectNode();
        p.put("type", "object");
        ObjectNode props = p.putObject("properties");
        props.putObject("keywords").put("type", "string").put("description", "搜索关键词");
        props.putObject("city").put("type", "string").put("description", "城市名或拼音（可选）");
        props.putObject("limit").put("type", "integer").put("description", "最多返回条数（可选）");
    ...
    @Override public String execute(JsonNode args) throws Exception {
        ...
        if (keywords.isBlank()) throw new IllegalArgumentException("keywords 参数不能为空");
        List<AmapService.PoiInfo> pois = amapService.searchPoi(keywords, null, city, limit);
        if (pois.isEmpty()) return "未找到相关地点，请尝试更换关键词或补充城市名。";
