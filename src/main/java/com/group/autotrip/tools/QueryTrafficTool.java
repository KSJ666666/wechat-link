...
public class QueryTrafficTool implements FunctionTool {
    ...
    public QueryTrafficTool(AmapService amapService, CityTransportSupport citySupport) { this.amapService = amapService; this.citySupport = citySupport; }
    @Override public String name() { return "query_traffic"; }
    @Override public String description() { return "查询某条道路的实时交通路况（是否拥堵）。"; }
    @Override public JsonNode parameters() {
        ObjectNode p = MAPPER.createObjectNode();
        p.put("type", "object");
        ObjectNode props = p.putObject("properties");
        props.putObject("city").put("type", "string").put("description", "城市名（必填）");
        props.putObject("road").put("type", "string").put("description", "道路名称（必填）");
    ...
    @Override public String execute(JsonNode args) throws Exception {
        ...
        if (city.isBlank() || road.isBlank()) throw new IllegalArgumentException("city 和 road 参数不能为空");
        String cityCode = citySupport.cityCode(city);
        if (cityCode.isBlank()) cityCode = city;
