...
public class AmapService {
    ...
    public RouteOption getDrivingRouteOption(PoiInfo from, PoiInfo to) throws IOException {
        ...
        return new RouteOption(
                ...
                from.name(),
                to.name(),
                safeLong(path.path("distance").asText("")),
                safeLong(path.path("duration").asText("")),
    ...
    /**
     * 查询两个地点之间的公交/地铁/火车方案。
     * 返回按交通方式归并后的最优方案，可能包含公交、地铁、高铁/火车。
     */
     ...
     *
     * @param origin       起点地点
     * @param destinations 目的地地点列表
     * @param city         统一城市（可选，用于准确定位）
     * @param driving      true=驾车距离（含预计耗时），false=直线距离
     * @return 距离结果，驾车/直线模式均按距离由近到远排列，定位失败的目的地放在末尾
     */
    public List<DistanceInfo> getDistanceMatrix(
            String origin, List<String> destinations, String city, boolean driving) throws IOException {
        requireKey();
        List<String> uniqueNames = destinations.stream()
                .map(String::trim)
                .filter(s -> !s.isBlank())
                .distinct()
                .limit(MAX_MATRIX_DESTINATIONS)
                .toList();
        ...
        String origins = resolved.stream()
                .map(PoiInfo::location)
        ...
        distances.sort(Comparator.comparingDouble(DistanceInfo::distanceKm)
                .thenComparing(DistanceInfo::name));
    ...
    /** POI 搜索失败时用地名直接转城市中心坐标（支持“上海”这类城市名）。 */
    private PoiInfo geocodePlace(String name, String city) throws IOException {
        ...
        return new PoiInfo(
                ...
                name,
                "",
                geo.path("formatted_address").asText(""),
                ...
                if (mode == TransportMode.RAIL) {
                    ...
                    if (busLines.isArray()) {
                        for (JsonNode busLine : busLines) {
                            ...
                            if ((mode == TransportMode.METRO && !isMetro)
                                    || (mode == TransportMode.BUS && isMetro)) {
                                continue;
                            }
    ...
    /** 高德并发限流：最多 2 个请求同时执行，QPS 超限时自动重试一次。 */
    ...
    public record DistanceInfo(String name, double distanceKm, int durationMin, String note) {
        ...
        public String toString() {
            ...
            if (durationMin > 0) {
                return String.format("%s：约 %.1f 公里，约 %d 分钟", name, distanceKm, durationMin);
            }
