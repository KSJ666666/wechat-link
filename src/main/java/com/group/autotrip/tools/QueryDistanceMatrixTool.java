...
public class QueryDistanceMatrixTool implements FunctionTool {
    ...
    public QueryDistanceMatrixTool(AmapService amapService) {
        this.amapService = amapService;
    }

    @Override
    public String name() {
        return "query_distance_matrix";
    }

    @Override
    public String description() {
        return "一次计算一个起点到多个目的地的距离和预计耗时，用于比较哪些地点顺路、为行程选点排序。"
                + "当用户需要比较多个景点/地点离起点远近、想知道哪个更近时调用。";
    }
    ...
    public JsonNode parameters() {
        ...
        props.putObject("origin")
                .put("type", "string")
                .put("description", "起点地点（必填，建议包含城市名，如：北京天安门）");
        ObjectNode destination = props.putArray("destinations").addObject();
        destination.put("type", "string")
                .put("description", "目的地地点列表（必填，建议包含城市名，如：北京故宫、北京景山公园），最多 50 个");
        props.putObject("city")
                .put("type", "string")
                .put("description", "地点所在城市（可选，用于准确定位，如：北京）");
        ObjectNode mode = props.putObject("mode");
        mode.put("type", "string");
        mode.put("description", "driving=驾车距离（默认，含预计耗时），straight=直线距离");
    ...
    public String execute(JsonNode args) throws Exception {
        ...
        if (dests.isArray()) {
            for (JsonNode d : dests) {
                String value = d.asText("");
                if (!value.isBlank()) {
                    destinations.add(value);
                }
            }
        }
        if (origin.isBlank() || destinations.isEmpty()) {
            throw new IllegalArgumentException("origin 和 destinations 参数不能为空");
        }
        ...
        int number = 1;
        for (AmapService.DistanceInfo info : distances) {
            sb.append(number++).append(". ").append(info).append('\n');
        }
