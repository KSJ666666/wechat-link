...
public class QueryAttractionDetailTool implements FunctionTool {
    ...
    public QueryAttractionDetailTool(AmapService amapService) {
        this.amapService = amapService;
    }

    @Override
    public String name() {
        return "query_attraction_detail";
    }

    @Override
    public String description() {
        return "查询单个景点的详细信息（评分、景区等级、开放时间、电话、地址），用于生成攻略中的景点介绍。"
                + "当用户需要了解某个具体景点的开放时间、评分、等级或联系方式时调用。";
    }
    ...
    public JsonNode parameters() {
        ...
        props.putObject("city")
                .put("type", "string")
                .put("description", "景点所在城市（必填，如：北京）");
        props.putObject("name")
                .put("type", "string")
                .put("description", "景点名称（必填，如：故宫博物院）");
    ...
    public String execute(JsonNode args) throws Exception {
        ...
        if (city.isBlank() || name.isBlank()) {
            throw new IllegalArgumentException("city 和 name 参数不能为空");
        }
